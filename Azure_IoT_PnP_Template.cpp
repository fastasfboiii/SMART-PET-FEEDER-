...
//#include "Azure_IoT_Central_ESP32.ino"
#include <stdarg.h>
#include <stdlib.h>
#include <DHT.h>
...
#define TELEMETRY_PROP_NAME_FOOD "food"
#define TELEMETRY_PROP_NAME_WATER "water"
...
static size_t telemetry_frequency_in_seconds = 5000; // With default frequency of once in 10 seconds.
...
void check_food(){
  ...
  if (irValue == LOW) { // Assuming LOW means it detects food
    delay(50);
    food = 1; // there is food
  } 
  else {
    delay(50);
    food = 0; // there is no food
  }
}
    ...
    size_t* payload_buffer_length)
{
  ...
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_FOOD));
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding food property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, food, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding food property value to telemetry payload. ");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_WATER));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding water property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, water, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding water property value to telemetry payload. ");
